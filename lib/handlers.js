 ...
 * target channel.
 */
listnerHandlers.onMessageHandler = function onMessageHandler(user, message) {
  ...
  const queuedResponse = responseState.response;
  const queuedRequest = responseState.request;
  const queuedTarget = responseState.target;
  ...
  const listenedMessage = message;
  if (message.length === (queuedResponse.length + bypassChar.length)) {
    if (message.substring(message.length - bypassChar.length) === bypassChar)
      message = message.substring(0, message.length - bypassChar.length);
  }
  if (message !== queuedResponse) return;


  logger.info(`\n* Executed "${queuedRequest.join(" ")}" command`);
  logger.info("* Details:", { queuedTarget, queuedResponse });

  const messageState = user.getMessageState();
  messageState.nextMessageState(listenedMessage, Date.now());
